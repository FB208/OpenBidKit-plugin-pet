Create Codex pet row `running-right` for `xiaoyi`: exactly 8 full-body frames in one horizontal strip on flat pure yellow #FFFF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: 保持用户确认的旋涡云螺精灵造型：蓝青渐变大旋涡背部与尾部、友善面孔、两只短手和两只圆脚、精致2.5D绘本贴纸质感。角色必须适合160px桌宠显示。. Preserve silhouette, face, palette, material, proportions, markings, and props.
